...
function Dashboard() {
  ...
  const fetchDogs = async () => {
    try {
      ...
      const { data, error } = await supabase.from("Godfather Kennel").select("*");
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto">
        <Button className='mb-5 p-5' onClick={()=>navigate('/add-dog')}>Add Dog</Button>
  ...
  {loading ? (
    ...
    currentDogs.map((dog) => (
      <TableRow key={dog.id}>
        ...
        <TableCell>{new Date(dog.date_of_birth).toDateString()}</TableCell>
        ...
        <TableCell>
          <Popover>
            ...
            <PopoverContent className="w-32 p-2 text-sm">
              <div className="hover:bg-gray-100 p-1 rounded cursor-pointer flex gap-2 items-center" onClick={() => navigate(`/dog/${dog.unique_id}`)}>
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button
              variant="destructive"
              onClick={() => {
                setDogs((prev) => prev.filter((dog) => dog.id !== confirmDeleteId));
